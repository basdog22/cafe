...
/**output all the food items of each type of food*/
		while($row_fcata = $mysql->fetch($result_fcata)) {
            ...
            while($row_finfo = $mysql->fetch($result_finfo)) {
                echo "<tr>
						<td>".$row_finfo['food_id']."</td>
						<td>".$row_finfo['food_name']." </td>
						<td>&#165;".$row_finfo['price']." </td>
						<td onmousemove='vis({$row_finfo['food_id']})' onmouseout='hide({$row_finfo['food_id']});check({$row_finfo['food_id']})'>
							...
							<button id='r{$row_finfo['food_id']}' class='bnum' type='button' onclick='a({$row_finfo['food_id']})' ><b>+</b></button>
						</td>
					</tr>";
            }
		}		
